codeunit 27003001 "Document Approval PR-IBIZPR"
{
    ...
    var
        ...
        DocApp: Codeunit "Approvals Management-IBIZPR";
        DocAppmail: Codeunit "Approvals Mgt Noti.-IBIZPR";
        GTotalAmt: Decimal;
        GLSetup: Record "General Ledger Setup";
        PRFunc: Codeunit "PR Functions-IBIZPR";
        Text001: Label 'PR %2 requires further approval to make %1.\\Approval request entries have been created.';
        ...
        Text005: Label 'User ID %1 does not exist in the Document Approval Setup PR.';
    ...
    procedure SendPurchaseReqApproval(var PRHeader: Record "PR Header"; TotalBudget: Decimal): Boolean;
    var
    ...
    begin
        with PRHeader do begin
            ...
            if not ApprovalSetup.GET then
                ERROR(Text004);
            if not PurchaseReqLinesexist(PRHeader) then
                ERROR(Text015, FORMAT("PR Document Type"), "No.");
            ...
            TemplateRec.SETRANGE("No. series", PRHeader."No. series");
            ...
            if TemplateRec.FIND('-') then begin
                repeat
                    if TemplateRec."Direct Approver" then begin
                        if not FindApproverPurchaseReqDirect(PRHeader, ApprovalSetup, TemplateRec) then
                            ERROR(Text010);
                    end else begin
                        if not FindApproverPurchaseReq(PRHeader, ApprovalSetup, TemplateRec) then
                            ERROR(Text010);

                    end;
                until TemplateRec.NEXT = 0;
                FinishApprovalEntryPurchaseReq(PRHeader, ApprovalSetup, MessageType, TotalBudget);
                case MessageType of
                    MessageType::AutomaticPrePayment:
                        MESSAGE(Text128, PRHeader."PR Document Type", PRHeader."No.");
                    MessageType::AutomaticRelease:
                        MESSAGE(Text003, PRHeader."PR Document Type", PRHeader."No.");
                    MessageType::RequiresApproval:
                        MESSAGE(Text001, PRHeader."PR Document Type", PRHeader."No.");
                end;
            end else
    ...
    procedure SendPurchaseReqApprovalLOA(var PRHeader: Record "PR Header"; Totalbudget: Decimal): Boolean;
    var
        TemplateRec: Record "Approval Templates";
        ApprovalSetup: Record "Approval Setup";
        MessageType: Option " ",AutomaticPrePayment,AutomaticRelease,RequiresApproval;
    begin
        with PRHeader do begin
            ...
            if TemplateRec.FIND('-') then begin
                repeat
                    if TemplateRec."Direct Approver" then begin
                        if not FindApproverPurchaseReqDirLOA(PRHeader, ApprovalSetup, TemplateRec) then
                            ERROR(Text010);
                    end else begin
                        if not FindApproverPurchaseReqLOA(PRHeader, ApprovalSetup, TemplateRec) then
                            ERROR(Text010);

                    end;
                until TemplateRec.NEXT = 0;
                FinishApprovalEntryPurchReqLOA(PRHeader, ApprovalSetup, MessageType, Totalbudget);
                case MessageType of
                    MessageType::AutomaticPrePayment:
                        MESSAGE(Text128, PRHeader."PR Document Type", PRHeader."No.");
                    MessageType::AutomaticRelease:
    ...
    procedure PurchaseReqLinesexist(var PRHeader: Record "PR Header"): Boolean;
    var
    ...
    procedure FindApproverPurchaseReq(PRHeader: Record "PR Header"; ApprovalSetup: Record "Approval Setup"; AppTemplate: Record "Approval Templates"): Boolean;
    var
                    ...
                    case AppTemplate."Limit Type" of
                        AppTemplate."Limit Type"::"Approval Limits":
                            begin
                                ApproverId := UserSetup."User ID";
                                MakeApprovalEntryPurchaseReq(
                                   DATABASE::"PR Header", PRHeader."No.", PRHeader."Purchaser Code",
                                   ApprovalSetup, ApproverId, AppTemplate."Approval Code", UserSetup, ApprovalAmount, ApprovalAmountLCY,
                                   AppTemplate, 0);
                                if not UserSetup."Unlimited Purchase Approval" and
                                   ((ApprovalAmountLCY > UserSetup."Purchase Amount Approval Limit") or
    ...
    procedure FindApproverPurchaseReqLOA(PRHeader: Record "PR Header"; ApprovalSetup: Record "Approval Setup"; AppTemplate: Record "Approval Templates"): Boolean;
    var
        UserSetup: Record "Document Approval Setup";
        ApproverId: Code[50];
        ApprovalAmount: Decimal;
    ...
    procedure FindApproverPurchaseReqDirect(PRHeader: Record "PR Header"; ApprovalSetup: Record "Approval Setup"; AppTemplate: Record "Approval Templates"): Boolean;
    var
        LText001: Label '"User setup does not exist for requester %1 "';
        LText002: Label 'Requester should not be empty in the Purchase Requisition header';
        ApprovalAmount: Decimal;
    ...
    procedure FindApproverPurchaseReqDirLOA(PRHeader: Record "PR Header"; ApprovalSetup: Record "Approval Setup"; AppTemplate: Record "Approval Templates"): Boolean;
    var
        LText001: Label '"User setup does not exist for requester %1 "';
        LText002: Label 'Requester should not be empty in the Purchase Requisition header';
        ApprovalAmount: Decimal;
    ...
    begin
        with ApprovalEntry do begin
            ...
            "Document No." := DocNo;
            if PRHeader.GET(DocNo) then
                ApprovalEntry."Record ID to Approve" := PRHeader.RECORDID;
    ...
    procedure CalcPurchaseReqDocAmount(PRHeader: Record "PR Header"; var ApprovalAmount: Decimal; var ApprovalAmountLCY: Decimal);
    var
    ...
    procedure FinishApprovalEntryPurchaseReq(PRHeader: Record "PR Header"; ApprovalSetup: Record "Approval Setup"; var MessageID: Option " ",AutomaticPrePayment,AutomaticRelease,RequiresApproval; TotalBudget: Decimal);
    var
        DocReleased: Boolean;
        ApprovalEntry: Record "Approval Entry";
    ...
    begin
        ...
        with ApprovalEntry do begin
            ...
            SETRANGE("Document No.", PRHeader."No.");
                    ...
                    if "Sender ID" = "Approver ID" then begin
                        ...
                        if not IsOpenStatusSet then begin
                            ...
                            if ApprovalSetup.Approvals then
                                ApprovalsMgtNotification.SendPRApprovalsMail(PRHeader, ApprovalEntry, TotalBudget);
                        end;
                until NEXT = 0;
            ...
            if not IsOpenStatusSet then begin
                SETRANGE(Status);
                FINDLAST;
                DocReleased := ApprovalRelease(ApprovalEntry, PRHeader);
            end;
    ...
    procedure FinishApprovalEntryPurchReqLOA(PRHeader: Record "PR Header"; ApprovalSetup: Record "Approval Setup"; var MessageID: Option " ",AutomaticPrePayment,AutomaticRelease,RequiresApproval; TotalBudget: Decimal);
    var
        DocReleased: Boolean;
        ApprovalEntry: Record "Approval Entry";
        ApprovalsMgtNotification: Codeunit "Approvals Mgt Noti.-IBIZPR";
    ...
    begin
        ...
        with ApprovalEntry do begin
            ...
            if not IsOpenStatusSet then begin
                SETRANGE(Status);
                FINDLAST;
                DocReleased := ApprovalReleaseLOA(ApprovalEntry, PRHeader);
            end;

            if DocReleased then begin
                MessageID := MessageID::AutomaticRelease;
            end else begin
                PRHeader."LOA Status" := PRHeader."LOA Status"::"Pending Approval";
                PRHeader.MODIFY(true);
                PRline.RESET;
                PRline.SETRANGE("Document No.", PRHeader."No.");
                if PRline.FINDSET then
    ...
    procedure ApprovalRelease(ApprovalEntry: Record "Approval Entry"; PRHeader: Record "PR Header"): Boolean;
    var
        ...
        if ApprovalEntry."Table ID" <> 0 then begin
            ...
            if NextApprovalEntry.FIND('-') then begin
                if NextApprovalEntry.Status = NextApprovalEntry.Status::Open then
                    ...
                    ApprovalSetup.GET;
                    if ApprovalSetup.Approvals then
                        ApprovalsMgtNotification.SendPRApprovalsMail(PRHeader, NextApprovalEntry, PRHeader."PR Available Budget");
                    exit(false);
                end;
            end else begin
                if ApprovalSetup.GET and ApprovalSetup.Approvals then begin
                    if PRHeader."PR Document Type" = PRHeader."PR Document Type"::PC then
                        ApprovalsMgtNotification.SendPRPCApprovedMail(PRHeader, ApprovalEntry);
                end;
                PRFunc.ReleasePRDocAfterApprove(PRHeader);
                exit(true);
            end;
        end;
    end;

    procedure ApprovalReleaseLOA(ApprovalEntry: Record "Approval Entry"; PRHeader: Record "PR Header"): Boolean;
    var
        NextApprovalEntry: Record "Approval Entry";
        ApprovalSetup: Record "Approval Setup";
        ApprovalsMgtNotification: Codeunit "Approvals Mgt Noti.-IBIZPR";
    begin
        if ApprovalEntry."Table ID" <> 0 then begin
            ...
            if NextApprovalEntry.FIND('-') then begin
                ...
                PRFunc.ReleasePRDocAfterApproveLOA(PRHeader);
                exit(true);
            end;
        end;
    end;
    ...
    begin
        if ApprovalEntry."Table ID" <> 0 then begin
            ApprovalSetup.GET;
            PRHeader.RESET;
            PRHeader.SETRANGE("No.", ApprovalEntry."Document No.");
            ...
            if ApprovalSetup.Rejections then
                DocAppmail.SendPRRejectionsMail(PRHeader, ApprovalEntry);
    ...
    begin
        if ApprovalEntry."Table ID" <> 0 then begin
            ...
            PRHeader."LOA Status" := PRHeader."LOA Status"::Open;
            PRHeader.MODIFY;
            MESSAGE(GText005, PRHeader."No.");
        end;
    end;
    ...
    procedure CancelPurchaseApprovalRequest(PRHeader: Record "PR Header"): Boolean;
    var
        ApprovalEntry: Record "Approval Entry";
        ApprovalSetup: Record "Approval Setup";
        ...
        PRLine: Record "PR Line";
    begin
        if (PRHeader.Status = PRHeader.Status::"Pending Approval") then begin
            ...
            with PRHeader do begin
                ...
                MESSAGE(Text002, PRHeader."PR Document Type", PRHeader."No.");
            end;
        end
        else
            MESSAGE(Text130)
    ...
    procedure FindApproverRFQ(PRHeader: Record "RFQ Comparison"; ApprovalSetup: Record "Approval Setup"; AppTemplate: Record "Approval Templates"): Boolean;
    var
        UserSetup: Record "Document Approval Setup";
        ApproverId: Code[20];
        ApprovalAmount: Decimal;
    ...
    begin
        ...
        CalcRFQDocAmount(PRHeader, ApprovalAmount, ApprovalAmountLCY);
                        ...
                        AppTemplate."Limit Type"::"Approval Limits":
                            begin
                                ...
                                MakeApprovalEntryRFQ(
                                   DATABASE::"RFQ Comparison", PRHeader."No.", '',
                                   ApprovalSetup, ApproverId, AppTemplate."Approval Code", UserSetup, ApprovalAmount, ApprovalAmountLCY,
                                   AppTemplate, 0);
                                if not UserSetup."Unlimited Purchase Approval" and
                                   ((ApprovalAmountLCY > UserSetup."Purchase Amount Approval Limit") or
    ...
    procedure CalcRFQDocAmount(PRHeader: Record "RFQ Comparison"; var ApprovalAmount: Decimal; var ApprovalAmountLCY: Decimal);
    var
        LPLine: Record "Purchase Line";
    begin
        LPLine.RESET;
        LPLine.SETRANGE("Create PO", true);
        LPLine.SETRANGE(LPLine."RFQ No.", PRHeader."No.");
