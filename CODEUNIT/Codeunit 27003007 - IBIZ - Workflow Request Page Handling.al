codeunit 27003007 "IBIZ - WF Req. Hdle.al-IBIZPR"
{
    //CU-1502---Start------------------------
    // procedure InsertPRDocumentApprovalWorkflow(VAR Workflow: Record Workflow; DocumentType: Option; ApproverType: Option; LimitType: Option; WorkflowUserGroupCode: Code[20]; DueDateFormula: DateFormula)//Version 19.0.0.0>>
    ...
    begin
        InsertWorkflow(Workflow, GetWorkflowCode(PurchReqApprWorkflowCodeTxt), PurchReqApprWorkflowDescTxt, PRDocCategoryTxt);

        PopulateWorkflowStepArgument(WorkflowStepArgument, ApproverType, LimitType, 0,
          WorkflowUserGroupCode, DueDateFormula, TRUE);

        WorkflowSetup.InsertDocApprovalWorkflowSteps(Workflow, BuildPRHeaderTypeConditions(PRHeader.Status::Open),
          WorkflowEventHandling.RunWorkflowOnSendPRDocForApprovalCode,
          BuildPRHeaderTypeConditions(PRHeader.Status::"Pending Approval"),
          WorkflowEventHandling.RunWorkflowOnCancelPRApprovalRequestCode,
          WorkflowStepArgument, TRUE);
    end;

    // procedure InsertPRLOADocumentApprovalWorkflow(VAR Workflow: Record Workflow; DocumentType: Option; ApproverType: Option; LimitType: Option; WorkflowUserGroupCode: Code[20]; DueDateFormula: DateFormula)//Version 19.0.0.0>>
    ...
    begin
        ...
        PopulateWorkflowStepArgument(WorkflowStepArgument, ApproverType, LimitType, 0,
          WorkflowUserGroupCode, DueDateFormula, TRUE);
        WorkflowSetup.InsertDocApprovalWorkflowSteps(Workflow, BuildPRHeaderTypeConditions(PRHeader."LOA Status"::Open),
    ...
    begin
        WorkflowSetup.InsertTableRelation(DATABASE::"PR Header", 0, DATABASE::"Approval Entry", ApprovalEntry.FIELDNO("Record ID to Approve"));
    end;

    [EventSubscriber(ObjectType::Codeunit, Codeunit::"Workflow Setup", 'OnAddWorkflowCategoriesToLibrary', '', false, false)]
    local procedure OnAddWorkflowCategoriesToLibrary()
    begin
        InsertWorkflowCategory(PRDocCategoryTxt, PRDocCategoryDescTxt);
        InsertWorkflowCategory(PRLOADocCategoryTxt, PRLOADocCategoryDescTxt);
    end;

    procedure PRApprovalWorkflowCode(): Code[17]
    begin
    ...
    begin
        ...
        InsertPRLOAApprovalWorkflowTemplate;
    end;
    ...
    procedure InsertPRApprovalWorkflowTemplate()
    var
        Workflow: Record "Workflow";
        WorkflowSetup: Codeunit "Workflow Setup";
    begin
    ...
    procedure InsertPRApprovalWorkflowDetails(VAR Workflow: Record Workflow)
    var
        PRHeader: Record "PR Header";
        WorkflowStepArgument: Record "Workflow Step Argument";
        BlankDateFormula: DateFormula;
    ...
    begin
        ...
        WorkflowSetup.InsertDocApprovalWorkflowSteps(Workflow,
          BuildPRHeaderTypeConditions(PRHeader.Status::Open),
          WorkflowEventHandling.RunWorkflowOnSendPRDocForApprovalCode,
          BuildPRHeaderTypeConditions(PRHeader.Status::"Pending Approval"),
          WorkflowEventHandling.RunWorkflowOnCancelPRApprovalRequestCode,
          WorkflowStepArgument, TRUE);
    ...
    procedure BuildPRHeaderTypeConditions(Status: Option): Text
    var
        PRHeader: Record "PR Header";
        PRLine: Record "PR Line";
    begin
        PRHeader.SETRANGE(Status, Status);
        EXIT(STRSUBSTNO(PurchReqHeaderTypeCondnTxt, Encode(PRHeader.GETVIEW(FALSE)), Encode(PRLine.GETVIEW(FALSE))));
    end;
    ...
    begin
        InsertReqPageEntity(PRDocumentCodeTxt, PRDocumentDescTxt, DATABASE::"PR Header", DATABASE::"PR Line");
        InsertReqPageEntity(PRLOADocumentCodeTxt, PRLOADocumentDescTxt, DATABASE::"PR Header", DATABASE::"PR Line");
        InsertPRHeaderReqPageFields;
        InsertPRLineReqPageFields;
        InsertPRLOAHeaderReqPageFields;
        InsertPRLOALineReqPageFields;
        AssignEntityToWorkflowEvent(DATABASE::"PR Header", PRDocumentCodeTxt);
        AssignEntityToWorkflowEvent(DATABASE::"PR Header", PRLOADocumentCodeTxt);
    end;

    procedure InsertPRHeaderReqPageFields()
    var
        PRHeader: Record "PR Header";
    begin
    ...
    procedure InsertPRLineReqPageFields()
    var
        PRLine: Record "PR Line";
    begin
    ...
    var
        PRDocumentCodeTxt: Label 'PRDOC';
        PRLOADocumentCodeTxt: Label 'PRLOADocumentCodeTxt';
        PRDocumentDescTxt: Label 'PR Document';
        PRLOADocumentDescTxt: Label 'PR LOA Document';
        PurchReqApprWorkflowCodeTxt: Label 'PRAPW';
        PurchReqApprWorkflowDescTxt: Label 'Purchase Requisition Approval Workflow';
        PRDocCategoryTxt: Label 'PRDOC';
        PRDocCategoryDescTxt: Label 'PR Documents';
        PurchReqHeaderTypeCondnTxt: Label '<?xml version="1.0" encoding="utf-8" standalone="yes"?><ReportParameters><DataItems><DataItem name="PR Header">%1</DataItem><DataItem name="PR Line">%2</DataItem></DataItems></ReportParameters>';
