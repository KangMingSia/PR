codeunit 27003000 "PR Functions-IBIZPR"
{
    ...
    begin
        ...
        foreach OrderNo in OrderNoList do
            if OrderPurchaseHeader.Get(OrderPurchaseHeader."Document Type"::Order, OrderNo) then begin
                PurchaseHeader."RFQ No" := OrderPurchaseHeader."RFQ No";
                PurchaseHeader."USERID PR to PQ" := OrderPurchaseHeader."USERID PR to PQ";
                PurchaseHeader."USERID PR to PO" := OrderPurchaseHeader."USERID PR to PO";
                PurchaseHeader."USERID PQ to PO" := OrderPurchaseHeader."USERID PQ to PO";
                PurchaseHeader."WBS ID" := OrderPurchaseHeader."WBS ID";
                PurchaseHeader."Activity ID" := OrderPurchaseHeader."Activity ID";
                PurchaseHeader."Budgetary PR" := OrderPurchaseHeader."Budgetary PR";
                PurchaseHeader."PR No." := OrderPurchaseHeader."PR No.";
                PurchaseHeader."PR Line No." := OrderPurchaseHeader."PR Line No.";
                // "Converted To PO"
                // "Prospect Vendor"
                PurchaseHeader."PR Requester" := OrderPurchaseHeader."PR Requester";
                PurchaseHeader.Modify(false);
            end;
    end;
    // [EventSubscriber(ObjectType::Codeunit, Codeunit::"User Login Time Tracker", 'OnAfterCreateorUpdateLoginInfo', '', false, false)]
    ...
    begin
        ...
        if lSuggestedVendor.FINDSET then begin
            if PRCode = '' then begin
                ...
                lPReqQuote."Purchaser Code" := lPRHdr."Purchaser Code";
                ...
                if lPurchaseQuoteDescription.FINDSET then
                    repeat
                        ...
                        if not lpurchaseQuoteLineDesc.FINDFIRST then begin
                            ...
                            lpurchaseQuoteLineDesc."Vendor Due Date" := WorkDate();
                            lpurchaseQuoteLineDesc."RFQ Date" := WorkDate();
                ...
                repeat
                    if TempSuggestVend <> lPRLine."Suggested Vendor" then begin
                        ...
                        if PRLine2.FINDSET then begin
                            ...
                            lPurchaseHdr."USERID PR to PO" := Format(USERID);
                            lPurchaseHdr."Purchaser Code" := lPRHdr."Purchaser Code";
                            lPurchaseHdr."PR Requester" := lPRHdr.Requester;
                            ...
                            repeat
                                ...
                                //lPurchaseLine."Article Code" := PRLine2."Article Code";
                                lPurchaseLine.VALIDATE("Article Code", PRLine2."Article Code");
    ...
    procedure ReleasePRDocument(var PRHeader: Record "PR Header");
    var
        LSugVend: Record "Suggested Vendor";
        LPRLine: Record "PR Line";
        LApprovalTemp: Record "Approval Templates";
    ...
    begin
        CheckDim(PRHeader);
        GLSetup.GET;
        if PRHeader."PR Document Type" = PRHeader."PR Document Type"::PC then begin
            if PRHeader.Status = PRHeader.Status::Closed then
        ...
        Noseries.RESET;
        Noseries.SETRANGE(Code, PRHeader."No. series");
        if Noseries.FINDFIRST then;

        PRHeader.TESTFIELD(Requester);

        LPRLine.RESET;
        LPRLine.SETRANGE("Document No.", PRHeader."No.");
        if LPRLine.FINDSET then
            repeat
                LPRLine.TESTFIELD("Shortcut Dimension 1 Code", PRHeader."Shortcut Dimension 1 Code");
            until LPRLine.NEXT = 0;
        if ((not PRHeader."Budgetary PR") and (Noseries."Enable Project Budget" or Noseries."Enable GL Budget")) then begin
            LPRLine.RESET;
            LPRLine.SETRANGE("Document No.", PRHeader."No.");
        ...
        PRHeader.Status := PRHeader.Status::Released;
        PRHeader."Released By" := Format(USERID);
        PRHeader.MODIFY;
        ...
        if PRLine.FINDSET then
            repeat
                ...
                PRLine.MODIFY;
            until PRLine.NEXT = 0;
        MESSAGE(GText004, PRHeader."No.");
    end;

    procedure WFReleasePRDocument(var PRHeader: Record "PR Header");
    var
        LSugVend: Record "Suggested Vendor";
        LPRLine: Record "PR Line";
        LApprovalTemp: Record "Approval Templates";
    ...
    begin
        if PRHeader.Status = PRHeader.Status::Released then begin
            if PRHeader."PR Document Type" = PRHeader."PR Document Type"::PC then
                PRHeader."LOA Status" := PRHeader."LOA Status"::Closed
            else
                PRHeader."LOA Status" := PRHeader."LOA Status"::Released;
            PRHeader.MODIFY;

            ConvertToOrder(PRHeader);
            MESSAGE(GText004, PRHeader."No.");
        end else begin
            CheckDim(PRHeader);
            GLSetup.GET;
    ...
    var
        ...
        LDocApprovalPurchReq: Codeunit "Document Approval PR-IBIZPR";
        LSugVend: Record "Suggested Vendor";
        LAmountLCY: Decimal;
    ...
    begin
        ...
        PrHeaderRec.RESET;
        PrHeaderRec.SETRANGE("No.", RFQComparision."PR No");
        if PrHeaderRec.FINDFIRST then begin
            Noseries.RESET;
            Noseries.SETRANGE(Code, PrHeaderRec."No. series");
            if Noseries.FINDFIRST then;
            PurchasePaysetup.GET;
            GLSetup.GET;
            if ((not PrHeaderRec."Budgetary PR") and Noseries."Enable Project Budget") then begin
                CLEAR(TotalBudget);
                LPLine.RESET;
                LPLine.SETRANGE(LPLine."RFQ No.", RFQComparision."No.");
                LPLine.SETFILTER(LPLine.Type, '<>%1', LPLine.Type::" ");
            ...
            if ((not PrHeaderRec."Budgetary PR") and Noseries."Enable GL Budget") then begin
    ...
    procedure SendApprovalRequest(var PRHeader: Record "PR Header");
    var
        LPRLine: Record "PR Line";
        LDocApprovalPurchReq: Codeunit "Document Approval PR-IBIZPR";
        LSugVend: Record "Suggested Vendor";
        ...
        ItemNo: Code[20];
    begin
        PRHeader.TESTFIELD(PRHeader.Requester);
        Noseries.RESET;
        Noseries.SETRANGE(Code, PRHeader."No. series");
        if Noseries.FINDFIRST then;
        PurchasePaysetup.GET;
        ...
        if PRHeader."PR Document Type" = PRHeader."PR Document Type"::PC then begin
            ...
            ApprovalTemp.SETRANGE(ApprovalTemp."No. series", PRHeader."No. series");
        ...
        GLSetup.GET;
        if ((not PRHeader."Budgetary PR") and Noseries."Enable Project Budget") then begin
            if (PRHeader."Shortcut Dimension 1 Code" = '') then
                ERROR('%1 Should not be empty', GLSetup."Shortcut Dimension 1 Code");
            if (PRHeader."Shortcut Dimension 2 Code" = '') then
                ERROR('%1 Should not be empty', GLSetup."Shortcut Dimension 2 Code");
        ...
        //GL Budget
        if ((not PRHeader."Budgetary PR") and Noseries."Enable GL Budget") then begin
            if (PRHeader."Shortcut Dimension 1 Code" = '') then
                ERROR('%1 Should not be empty', GLSetup."Shortcut Dimension 1 Code");
            if (PRHeader."Shortcut Dimension 2 Code" = '') then
                ERROR('%1 Should not be empty', GLSetup."Shortcut Dimension 2 Code");
        ...
        LDocApprovalPurchReq.SendPurchaseReqApproval(PRHeader, TotalBudget);

    end;

    procedure ReopenPRDocument(var PRHdr: Record "PR Header");
    begin
    ...
    procedure WFCancelPRDocument(var PRHdr: Record "PR Header");
    begin
        if (PRHdr.Status = PRHdr.Status::Closed) then
            ERROR('PR Status Close can not Cancel');
    ...
    begin
        ...
        PurchaseHdr."USERID PR to PQ" := Format(USERID);
        PurchaseHdr."PR Requester" := LPRHdr.Requester;
    ...
    begin
        ...
        if PRLine1.FINDSET then begin
            ...
            PurchaseHdr."USERID PR to PQ" := Format(USERID);
            PurchaseHdr."PR Requester" := PR_Hdr.Requester;
    ...
    begin
        ...
        PurchaseHdr."PR No." := LPRHdr."No.";
        PurchaseHdr."USERID PR to PO" := Format(USERID);
        PurchaseHdr."PR Requester" := LPRHdr.Requester;
        PurchaseHdr."Purchaser Code" := LPRHdr."Purchaser Code";
        ...
        if PRLine1.FINDSET then begin
            ...
            repeat
                ...
                if (PRLine1.Type <> PRLine1.Type::" ") then begin//19.0.0.5
                    ...
                    PurchaseLine."Shortcut Dimension 7 Code" := PRLine1."Shortcut Dimension 7 Code";
                    PurchaseLine."Shortcut Dimension 8 Code" := PRLine1."Shortcut Dimension 8 Code";
                    PurchaseLine.Validate("Article Code", PRLine1."Article Code");
                    //19.0.0.7<<
                    PurchaseLine.VALIDATE("Location Code", PRLine1."Delivery Location");
    ...
    var
        lRFQComparison: Record "RFQ Comparison";
        lPurchaseLine: Record "Purchase Line";
        LPurchaseHdr: Record "Purchase Header";
        ...
        LPRHeaderForNoSeries: Record "PR Header";
        LNoSeries: Record "No. Series";
        LNoseriesMgmt: Codeunit NoSeriesManagement;
    ...
    begin
                ...
                if LPurchaseHdr.FINDFIRST then begin
                    ...
                    if LPL.FINDSET then begin
                        ...
                        if ExistingOrder.FINDFIRST then begin
                            ...
                            LPhdr."No. Printed" := 0;
                            LPRHeaderForNoSeries.RESET;
                            LPRHeaderForNoSeries.SETRANGE("No.", lPurchaseLine."PR No.");
                            if LPRHeaderForNoSeries.FINDFIRST then begin
                                LNoSeries.RESET;
                                LNoSeries.SETRANGE(Code, LPRHeaderForNoSeries."No. series");
                            ...
                            LPhdr."USERID PQ to PO" := Format(USERID);
                            LPhdr.INSERT(true);
                            LPhdr.COPYLINKS(LPurchaseHdr);

                            LPurchaseHdr."Converted To PO" := true;
                            LPurchaseHdr."USERID PQ to PO" := Format(USERID);
                            LPurchaseHdr."PR Requester" := LPhdr."PR Requester";
        ...
        if VerifyIfAllQtyConvertedToPO(RFQNo) then begin
            ...
            RFQNo.MODIFY;
            if PRHeader.GET(RFQNo."PR No") then begin
                PRHeader."PR Status" := PRHeader."PR Status"::Closed;
                PRHeader."PR Status" := PRHeader."PR Status"::Order;
                PRHeader.MODIFY;
            end;
        end;
        if lConverted then
    ...
    procedure ReleasePRDocAfterApprove(PRHeader: Record "PR Header");
    var
        LSugVend: Record "Suggested Vendor";
        LPRLine: Record "PR Line";
    begin
        if PRHeader."PR Document Type" = PRHeader."PR Document Type"::PC then begin
            PRHeader.Status := PRHeader.Status::Closed;
            PRHeader."PR Status" := PRHeader."PR Status"::Closed;
        end else begin
            PRHeader.Status := PRHeader.Status::Released;
        end;
        ...
        if PRLine.FINDSET then
            repeat
                if PRHeader."PR Document Type" = PRHeader."PR Document Type"::PC then
                    PRLine.Status := PRLine.Status::Closed
                else
                    PRLine.Status := PRLine.Status::Released;
                PRLine.MODIFY;
            until PRLine.NEXT = 0;
    ...
    procedure ReleasePRDocAfterApproveLOA(PRHeader: Record "PR Header");
    var
        LSugVend: Record "Suggested Vendor";
        LPRLine: Record "PR Line";
    begin
    ...
    begin
        if CONFIRM(Text007, true, PRNoRecPAR."No.") then begin
            ...
            PRArchHeaderLoc."Archive UserID" := Format(USERID);
            ...
            if PRArchHeaderLoc."No." <> '' then begin
                ...
                if PRLineLoc.FINDFIRST then
                    repeat
                        ...
                        PRArchLineLoc."Archive UserID" := Format(USERID);
    ...
    begin
        ...
        if PrHeaderRec.FINDFIRST then begin
            ...
            if ((not PrHeaderRec."Budgetary PR") and Noseries."Enable GL Budget") then begin
                ...
                LPLine.RESET;
                LPLine.SETRANGE(LPLine."RFQ No.", RFQComparision."No.");
                LPLine.SETFILTER(LPLine.Type, '%1', LPLine.Type::"G/L Account");
                LPLine.SETRANGE("Create PO", true);
                if LPLine.FINDSET then begin
                    repeat
                        CLEAR(TotalBudgetAmount);
                    ...
                    RFQComparision.MODIFY;
                    // 19.0.0.4>>
                    if IBIZApprovalsMgmt.CheckRFQCApprovalsWorkflowEnabled(RFQComparision) then
                        IBIZApprovalsMgmt.OnSendRFQCDocForApproval(RFQComparision);
                    // 19.0.0.4<<
                end else
                    ERROR('Create PO not Selected');
            end;
    ...
    procedure SendApprovalRequestLOA(var PRHeaderLoc: Record "PR Header");
    var
        LPRLine: Record "PR Line";
        LDocApprovalPurchReq: Codeunit "Document Approval PR-IBIZPR";
        LSugVend: Record "Suggested Vendor";
        ...
        AvailableBudget: Decimal;
    begin
        PRHeaderLoc.TESTFIELD(Status, PRHeaderLoc.Status::Released);
        PRHeaderLoc.TESTFIELD(PRHeaderLoc.Requester);
        PRHeaderLoc.TESTFIELD(PRHeaderLoc."Purchaser Code");

        CLEAR(TotalBudget);
        Noseries.RESET;
        Noseries.SETRANGE(Code, PRHeaderLoc."No. series");
        if Noseries.FINDFIRST then;
        PPSetup.GET;


        if PPSetup."Dimension for PR Approval" then begin
            if (PRHeaderLoc."Shortcut Dimension 1 Code" = '') then
                ERROR('%1 Should not be empty', GLSetup."Shortcut Dimension 1 Code");
            if (PRHeaderLoc."Shortcut Dimension 2 Code" = '') then
                ERROR('%1 Should not be empty', GLSetup."Shortcut Dimension 2 Code");
        end;

        LSugVend.RESET;
        LSugVend.SETRANGE("Document Type", LSugVend."Document Type"::"PR Header");
        LSugVend.SETRANGE("PR No.", PRHeaderLoc."No.");
        LSugVend.SETFILTER("Suggested Vendor", '<>%1', '');
        if LSugVend.FINDFIRST then begin
            ...
            PRLine.RESET;
            PRLine.SETRANGE("Document No.", PRHeaderLoc."No.");
            // PRLine.SETFILTER(PRLine.Type, '<>%1', PRLine.Type::Description);//19.0.0.5
            PRLine.SETFILTER(PRLine.Type, '<>%1', PRLine.Type::" ");//19.0.0.5
            if PRLine.FINDSET then
                repeat
        ...
        if PRHeaderLoc."PR Document Type" = PRHeaderLoc."PR Document Type"::PC then begin
            PRLine.RESET;
            PRLine.SETRANGE("Document No.", PRHeaderLoc."No.");
            // PRLine.SETFILTER(PRLine.Type, '<>%1', PRLine.Type::Description);//19.0.0.5
            PRLine.SETFILTER(PRLine.Type, '<>%1', PRLine.Type::" ");//19.0.0.5
            ...
            ApprovalTemp.RESET;
            ApprovalTemp.SETRANGE("Document Type", ApprovalTemp."Document Type"::PR);
            ApprovalTemp.SETRANGE(ApprovalTemp."No. series", PRHeaderLoc."No. series");
            if ApprovalTemp.FINDFIRST then begin
        ...
        //Project Budget
        GLSetup.GET;
        if ((not PRHeaderLoc."Budgetary PR") and Noseries."Enable Project Budget") then begin
            if (PRHeaderLoc."Shortcut Dimension 1 Code" = '') then
                ERROR('%1 Should not be empty', GLSetup."Shortcut Dimension 1 Code");
            if (PRHeaderLoc."Shortcut Dimension 2 Code" = '') then
                ERROR('%1 Should not be empty', GLSetup."Shortcut Dimension 2 Code");
        ...
        //GL Budget
        if ((not PRHeaderLoc."Budgetary PR") and Noseries."Enable GL Budget") then begin
        ...
        LDocApprovalPurchReq.SendPurchaseReqApprovalLOA(PRHeaderLoc, TotalBudget);

    end;

    procedure WFSendApprovalRequestLOA(var PRHeaderLoc: Record "PR Header");
    var
        LPRLine: Record "PR Line";
        LDocApprovalPurchReq: Codeunit "Document Approval PR-IBIZPR";
        LSugVend: Record "Suggested Vendor";
    ...
    begin
        ...
        if GPurchInvHdr.FINDSET then
            repeat
                ...
                GPurchInvLine.SETRANGE("Shortcut Dimension 1 Code", PrHeaderRec."Shortcut Dimension 1 Code");
    ...
    begin
        ...
        if GPurchCrHdr.FINDSET then
            repeat
                ...
                GPurchCrLine.SETRANGE("Shortcut Dimension 1 Code", PrHeaderRec."Shortcut Dimension 1 Code");
    ...
    begin
        GPRHeader.RESET;
        GPRHeader.SETRANGE("PR Date", StartDate, EndDate);
        if GPRHeader.FINDSET then
            repeat
                GPRLine.RESET;
                GPRLine.SETRANGE("Document No.", GPRHeader."No.");
    ...
    begin
        GPRHeader.RESET;
        GPRHeader.SETFILTER("PR Document Type", '<>%1', GPRHeader."PR Document Type"::PC);
        GPRHeader.SETRANGE("PR Date", StartDate, EndDate);
        GPRHeader.SETFILTER(Status, '%1|%2|%3', GPRLine.Status::Released, GPRHeader.Status::Closed, GPRHeader.Status::"Pending Approval");
        if GPRHeader.FINDSET then
            repeat
                GPRLine.RESET;
                GPRLine.SETRANGE("Document No.", GPRHeader."No.");
    ...
    begin
        ...
        GPRHeader.SETFILTER("PR Document Type", '<>%1', GPRHeader."PR Document Type"::PC);
        GPRHeader.SETRANGE("PR Date", StartDate, EndDate);
        GPRHeader.SETFILTER(Status, '%1|%2|%3', GPRHeader.Status::Released, GPRHeader.Status::Closed, GPRHeader.Status::"Pending Approval");
        GPRHeader.SETFILTER("LOA Status", '%1|%2|%3', GPRHeader.Status::Released, GPRHeader.Status::Closed, GPRHeader.Status::Open);
        if GPRHeader.FINDSET then
            repeat
                GPRLine.RESET;
                GPRLine.SETRANGE("Document No.", GPRHeader."No.");
                ...
                if GPRLine.FINDSET then
                    repeat
                        if (GPRLine.ConvertedtoOrder) then
                            BCalcPurchaseOrderForOnHoldPR(GPRHeader."No.", GPRLine)
                        else
                            if (GPRLine.ConvertedtoQuote) then
                                BCalcPurchaseQuoteForOnHoldPR(GPRHeader."No.", GPRLine)
                            else
                                GTotalBudgetOnHold += (GPRLine."Unit Cost (LCY)" * GPRLine.Quantity);
                    until GPRLine.NEXT = 0;
            until GPRHeader.NEXT = 0;
    ...
    local procedure CheckDim(PRHeaderPar: Record "PR Header");
    var
        PRLine2: Record "PR Line";
    begin
        PRLine2.RESET;
        PRLine2.SETRANGE("Document No.", PRHeaderPar."No.");
        // PRLine2.SETFILTER(Type, '<>%1', PRLine2.Type::Description);//19.0.0.5
        PRLine2.SETFILTER(Type, '<>%1', PRLine2.Type::" ");//19.0.0.5
        if PRLine2.FINDSET then begin
            repeat
                if (PRHeaderPar.Status = PRHeaderPar.Status::Open) then begin
                    CheckDimComb(PRLine2);
                    CheckDimValuePosting(PRHeaderPar, PRLine2);
                end
            until PRLine2.NEXT = 0;
        end;
    end;
    ...
    begin
        if PRLine."Line No." = 0 then
            if not DimMgt.CheckDimIDComb(PRHeader."Dimension Set ID") then
                ERROR(
                  Text032, PRHeader."No.", DimMgt.GetDimCombErr);

        if PRLine."Line No." <> 0 then
            if not DimMgt.CheckDimIDComb(PRLine."Dimension Set ID") then
                ERROR(
                  Text033,
                  PRHeader."No.", PRLine."Line No.", DimMgt.GetDimCombErr);
    end;

    local procedure CheckDimValuePosting(PRHeaderPar: Record "PR Header"; var PRLine2: Record "PR Line");
    var
        DimMgt: Codeunit DimensionManagement;
    ...
    begin
        if PRLine2."Line No." = 0 then begin
            ...
            NumberArr[1] := PRHeaderPar."Suggested Vendor";
            TableIDArr[2] := DATABASE::"Salesperson/Purchaser";
            NumberArr[2] := PRHeaderPar."Purchaser Code";

            if not DimMgt.CheckDimValuePosting(TableIDArr, NumberArr, PRHeaderPar."Dimension Set ID") then
                ERROR(
                  Text034,
                  PRHeaderPar."No.", DimMgt.GetDimValuePostingErr);
        end else begin
            // TableIDArr[1] := DimMgt.TypeToTableID3(PRLine2.Type); //TOCHECK
            NumberArr[1] := PRLine2."No.";

            if not DimMgt.CheckDimValuePosting(TableIDArr, NumberArr, PRLine2."Dimension Set ID") then
                ERROR(
                  Text035,
                  PRHeaderPar."No.", PRLine2."Line No.", DimMgt.GetDimValuePostingErr);
        end;
    ...
    begin
        if PRHdr.FindSet() then begin
            ...
            PurchaseHdr."USERID PR to PO" := Format(USERID);
            PurchaseHdr."PR Requester" := PRHdr.Requester;
