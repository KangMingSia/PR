codeunit 27003002 "Approvals Mgt Noti.-IBIZPR"
{
    ...
    procedure SetTemplatePR(AppEntry: Record "Approval Entry");
    var
        AppSetup: Record "Approval Setup";
        TempPath: Text[1000];
    begin
    ...
    procedure SendPRApprovalsMail(PurchaseReq: Record "PR Header"; ApprovalEntry: Record "Approval Entry"; TotalBudget: Decimal);
    var
        UserSetup: Record "User Setup";
        RecipientList: List of [Text];
    begin
        RecipientList.Add(Recipient);
        SetTemplatePR(ApprovalEntry);
        ...
        PurchaseReq.CALCFIELDS("PR Total Amount (LCY)", "PR Available Budget");
        while InStreamTemplate.EOS() = false do begin
            InStreamTemplate.READTEXT(InSReadChar, 1);
            if InSReadChar = '%' then begin
                EmailMessage.AppendToBody(Body);//20.0.0.0
                ...
                if (InSReadChar >= '0') and (InSReadChar <= '9') then begin
                    ...
                    Body := Body + InSReadChar;
                FillPRTemplateNew(Body, CharNo, PurchaseReq, ApprovalEntry, 0);
                EmailMessage.AppendToBody(Body);//20.0.0.0
                Body := InSReadChar;
            end else begin
                Body := Body + InSReadChar;
    ...
    procedure SendPRPCApprovedMail(PurchaseReq: Record "PR Header"; ApprovalEntry: Record "Approval Entry");
    var
        UserSetup: Record "User Setup";
        RecipientList: List of [Text];
    begin
        RecipientList.Add(Recipient);
        SetTemplatePR(ApprovalEntry);
        Subject := STRSUBSTNO(Text100, 'PR Approved');
        Body := Text013;
    ...
    procedure SendPRRejectionsMail(PRHeader: Record "PR Header"; ApprovalEntry: Record "Approval Entry");
    var
        AppCommentLine: Record "Approval Comment Line";
        Usersetup: Record "User Setup";
        RecipientList: List of [Text];
            ...
            while InStreamTemplate.EOS() = false do begin
                ...
                if InSReadChar = '%' then begin
                    ...
                    if (InSReadChar >= '0') and (InSReadChar <= '9') then begin
                        ...
                        Body := Body + InSReadChar;
                    FillPRTemplateNew(Body, CharNo, PRHeader, ApprovalEntry, 2);
                    EmailMessage.AppendToBody(Body);//20.0.0.0
                    Body := InSReadChar;
                end else begin
                    Body := Body + InSReadChar;
    ...
    procedure FillPRTemplateNew(var Body: Text[254]; TextNo: Text[30]; PRHeader: Record "PR Header"; AppEntry: Record "Approval Entry"; CalledFrom: Option Approve,Cancel,Reject,Delegate);
    begin
        case TextNo of
            '1':
                Body := STRSUBSTNO('Document Type :', '');
            '2':
                Body := STRSUBSTNO(Body, PRHeader."PR Document Type");
            '3':
                Body := STRSUBSTNO(Body, 'Document No. :');
            '4':
                Body := STRSUBSTNO(Body, PRHeader."No.");
            '5':
                Body := STRSUBSTNO(Body, 'Description :');
            '6':
                Body := STRSUBSTNO(Body, PRHeader.Description);
            '7':
                Body := STRSUBSTNO(Body, 'Expected Receipt date :');
            '8':
                Body := STRSUBSTNO(Body, PRHeader."Due Date");

            '9':
                Body := STRSUBSTNO(Body, 'Document Amount       :');
            '10':
                Body := STRSUBSTNO(Body, PRHeader."PR Total Amount");
            '11':
                Body := STRSUBSTNO(Body, 'Document Amount(LCY)  :');
            '12':
                Body := STRSUBSTNO(Body, PRHeader."PR Total Amount (LCY)");
        end;
    ...
    procedure SendPRBudgetNotificationMail(PurchaseReq: Record "PR Header"; TotalAmout: Decimal; "TotalAmount(LCY)": Decimal; TotalBudget: Decimal);
    var
        UserSetup: Record "User Setup";
        ...
        while InStreamTemplate.EOS() = false do begin
            ...
            if InSReadChar = '%' then begin
                ...
                if (InSReadChar >= '0') and (InSReadChar <= '9') then begin
                    ...
                    Body := Body + InSReadChar;
                FillPRBudgetTemplate(Body, CharNo, PurchaseReq, TotalAmout, "TotalAmount(LCY)", TotalBudget);
                EmailMessage.AppendToBody(Body);//20.0.0.0
                Body := InSReadChar;
            end else begin
                Body := Body + InSReadChar;
    ...
    procedure FillPRBudgetTemplate(var Body: Text[254]; TextNo: Text[30]; PRHeader: Record "PR Header"; TotalAmount: Decimal; TotalAmountLCY: Decimal; TotalBudget: Decimal);
    begin
        case TextNo of
            '1':
                Body := STRSUBSTNO('Document Type :', '');
